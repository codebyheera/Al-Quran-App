...
export default function BookmarksPage() {
  ...
  return (
    <div className="bookmarks-page page-enter">
      <Helmet>
        <title>{pageSeo.bookmarks.title}</title>
        <meta name="description" content={pageSeo.bookmarks.description} />
        <link rel="canonical" href={`https://alquranhub.org${pageSeo.bookmarks.path}`} />
      </Helmet>
          ...
          {bookmarks.map((b) => (
            <div key={b._id} className="bookmark-card card">
              ...
              <div className="flex-between mb-1">
                <Link to={`/surah/${encodeURIComponent(b.surahName)}#verse-${b.verseNumber}`} className="bm-ref">
              ...
              {/* Go-to link */}
              <Link to={`/surah/${encodeURIComponent(b.surahName)}#verse-${b.verseNumber}`} className="btn btn-secondary bm-goto mt-2">
                Read Surah →
              </Link>
            </div>
          ))}
